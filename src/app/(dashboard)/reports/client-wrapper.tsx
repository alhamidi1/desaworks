...
export function ReportsPageClient({ totalProjects, activeProjects, projectNames }: Props) {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl animate-fade-in">
      {/* Page header */}
      <div className="mb-8">
        <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-primary-500">
          {t('nav.reports')}
        </p>
        <h1 className="mt-1 text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight text-ink">
          {t('report.title')}
        </h1>
      ...
      <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {reportCards.map((card) => {
          ...
          if (card.projectNames !== undefined) {
            return (
              <div key={card.title} className="flex flex-col rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
                <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${card.iconBg} ${card.iconColor}`}>
                  {card.icon}
                </div>
                <h2 className="mt-4 text-base sm:text-lg font-bold text-ink">{card.title}</h2>
                <p className="mt-2 text-sm leading-6 text-ink-soft">{card.description}</p>

                {card.projectNames.length === 0 ? (
                  <p className="mt-4 text-xs text-neutral-400 italic">Belum ada proyek tersedia.</p>
                ) : (
                  <ul className="mt-4 space-y-1.5 flex-1">
                    {card.projectNames.slice(0, 5).map((p) => (
                      <li key={p.id}>
                        <Link
                          href={`/projects/${p.id}`}
                          className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-ink-soft transition-colors hover:bg-primary-50 hover:text-primary-500 group"
                        >
                          <span className="h-1.5 w-1.5 rounded-full bg-primary-500 opacity-50 group-hover:opacity-100 flex-shrink-0" />
          ...
          return (
            <Link key={card.title} href={card.href!} className="block">
              <div className="group flex h-full flex-col rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm transition-all duration-200 hover:shadow-md hover:border-primary-100 hover:-translate-y-0.5">
                <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${card.iconBg} ${card.iconColor}`}>
                  {card.icon}
                </div>
                <h2 className="mt-4 text-base sm:text-lg font-bold text-ink group-hover:text-primary-500 transition-colors">
                  {card.title}
                </h2>
                <p className="mt-2 flex-1 text-sm leading-6 text-ink-soft">
                  {card.description}
                </p>
                <div className="mt-4 flex items-center gap-1 text-sm font-semibold text-primary-500">
                  {t('report.viewReport')}
