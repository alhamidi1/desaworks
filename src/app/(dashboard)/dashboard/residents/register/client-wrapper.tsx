...
export function RegisterPageClient() {
  ...
  useEffect(() => {
    ...
    setWhatsappMsg(
      encodeURIComponent(
        t('registerPage.guideMessage', { link })
      )
    );
  }, [t]);
  ...
  return (
    <div className="mx-auto max-w-2xl animate-fade-in">
      {/* Header */}
      <div className="mb-6">
        <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-primary-500">{t('registerPage.category')}</p>
        <h1 className="mt-1 text-xl sm:text-2xl font-bold tracking-tight text-ink">{t('registerPage.title')}</h1>
      ...
      {tab === 'link' && (
        <div className="rounded-3xl border border-neutral-200 bg-white p-6 sm:p-8 shadow-sm space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-ink">{t('registerPage.shareTitle')}</p>
              <p className="text-xs text-ink-soft">{t('registerPage.shareSubtitle')}</p>
            </div>
          </div>

          {/* How it works */}
          <div className="rounded-2xl bg-neutral-50 p-4 space-y-3">
            <p className="text-xs font-bold uppercase tracking-wider text-neutral-400">{t('registerPage.howItWorks')}</p>
            {[
          ...
          <div className="space-y-2">
            <p className="text-xs font-bold uppercase tracking-wider text-neutral-400">{t('registerPage.linkTitle')}</p>
            <div className="flex items-center gap-2 rounded-xl border border-neutral-200 bg-neutral-50 p-3">
              ...
              >
                {copied ? t('registerPage.copied') : t('registerPage.copy')}
              </button>
            </div>
          </div>
